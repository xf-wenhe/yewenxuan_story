// find_dup_blocks_v2.js — Find duplicate blocks with lower threshold
...
for (let ch = 1; ch <= 1000; ch++) {
  ...
  // Build line map with exact match, lines >=10 chars
  const lineMap = {};
  ...
  // Find contiguous blocks of duplicates (>=2 consecutive dup lines)
...
console.log("\nTotal blocks: " + results.length + ", Chapters: " + new Set(results.map(r => r.ch)).size);
