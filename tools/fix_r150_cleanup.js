...
console.log('\nTotal CJK: ' + totalCjk.toLocaleString());
...
for (const v of VOLUMES) {
  ...
  if (!fs.existsSync(d)) continue;
  for (const f of fs.readdirSync(d).filter(x => x.endsWith('-polished.md'))) {
    remaining += fs.readFileSync(path.join(d, f), 'utf-8').split('目光凝滞').length - 1;
  }
}
console.log('Remaining 目光凝滞: ' + remaining);