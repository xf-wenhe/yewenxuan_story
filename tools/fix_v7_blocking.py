...
# -*- coding: utf-8 -*-
"""
Batch fix blocking AI-pattern findings for V7 chapters only.
Reads _v7_blocking_files.json.

Safe mechanical fixes only:
  1. not-is-comparison:  delete negation clause, keep affirmative verbatim
  2. reverse-not-is:     delete negation tail
  3. negation-parade:    collapse repeated negations
  4. voice-contrast:     delete "voice not loud...but" contrast setup
  5. trailer-ending:     delete "no one knows" preview trailer
  6. em-dash line:       -- (standalone) -> delete
  7. em-dash text:       em-dash -> comma or period (context-aware)

Rules:
  - NEVER add extra punctuation - preserve original ending
  - Skip content inside quotes (dialogue)
  - Skip compound-word prefixes

Usage:
  python tools/fix_v7_blocking.py --dry-run
  python tools/fix_v7_blocking.py
"""
import re
import os
import sys
...
# voice-contrast: 声音不高，却... / 他的声音不高，尾音却...
voice_contrast_re = re.compile(
    r'(?:他的)?声音不高[，,](?:尾音)?却'
)
# trailer-ending: 没人知道...
...
def fix_voice_contrast(text):
    """Replace 声音不高，却... → delete voice-contrast setup."""
    masked = mask_quoted(text)
    changes = []
    for m in voice_contrast_re.finditer(masked):
...
def fix_trailer_ending(text):
    """Replace 没人知道 → '' (drop trailer ending phrase)."""
    masked = mask_quoted(text)
    changes = []
    for m in trailer_ending_re.finditer(masked):
        changes.append((m.start(), m.end(), ''))
    result = masked
    for start, end, replacement in sorted(changes, reverse=True):
        result = result[:start] + replacement + result[end:]
...
def process_file(filepath, dry_run=True):
    ...
    modified = fix_voice_contrast(modified)
    modified = fix_trailer_ending(modified)
...
def main():
    ...
    v7_json = os.path.join(ROOT_DIR, '_v7_blocking_files.json')
    if not os.path.exists(v7_json):
        print("ERROR: _v7_blocking_files.json not found.")
        sys.exit(1)

    with open(v7_json, 'r', encoding='utf-8') as f:
        files = json.load(f)
