...
for (const volDir of VOLUMES) {
  ...
  for (const f of files) {
    ...
    for (const entry of REPL) {
      ...
      while (idx >= 0) {
        counters[pattern]++;
        text = text.slice(0, idx) + entry[1] + text.slice(idx + pattern.length);
        idx = text.indexOf(pattern, idx + entry[1].length);
        changed = true;
      }
    }
    if (changed) {
...
if (cjkDrops.length) {
  ...
  for (const [chNum] of cjkDrops) {
    ...
    let idx = -1;
    for (const re of [/（第d+章完）/, /（第[一二三四五六七八九十百千万零]+章完）/, /（本章完）/]) {
